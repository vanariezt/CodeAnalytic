...
class CI_Loader {
	...
	function widget($widgets = array())
	{
		...
		{	
			$widget = strtolower(str_replace(EXT, '', str_replace('_wi', '', $widget)).'_wi');		

			if (isset($this->_ci_widgets[$widget]))
			{
				continue;
			}

			if (file_exists(APPPATH.'widgets/'.$widget.EXT))
			{
				include_once(APPPATH.'widgets/'.$widget.EXT);	
			}
			else
			{
				if (file_exists(BASEPATH.'widgets/'.$widget.EXT))
				{
					include_once(BASEPATH.'widgets/'.$widget.EXT);	
				}
				else
				{
					show_error('Unable to load the requested file: widgets/'.$widget.EXT);
				}
			}
			
			$this->_ci_widgets[$widget] = TRUE;
			log_message('debug', 'Widget loaded: '.$widget);
		}		
	}
	 ...
	 * @return	void
	 */
	function widgets($widgets = array())
	{
		$this->widget($widgets);
	}
